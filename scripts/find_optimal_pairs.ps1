# PowerShell script to find optimal trading pairs for triangular arbitrage
...
$dataDir = "data\pairs_analysis_$timestamp"
New-Item -ItemType Directory -Path $dataDir -Force | Out-Null

Write-Host "`nStarting comprehensive pair analysis..."
Write-Host "Testing multiple base currencies and trading pairs"
Write-Host "Results will be saved to: $dataDir"

# Run the optimizer with different base currencies
$baseCurrencies = @("CAD", "USD", "EUR", "GBP", "JPY", "AUD")

foreach ($base in $baseCurrencies) {
    Write-Host "`nAnalyzing triangular paths with base currency: $base"
    
    # Run the optimizer for this base currency
    python main.py optimize --days 90 --base $base --count 10
    
    # Copy the results to our timestamped directory
    ...
    # Run a quick simulation with the top path
    Write-Host "Running quick simulation with top $base paths..."
    python main.py run --mode simulate --amount 100 --min-profit-pct 0.1 --max-capital 100 --position-size-pct 2 --max-positions 1 --interval 30 --log-level INFO --duration 300
    ...
    # Wait a bit to avoid rate limits between currencies
    Write-Host "Waiting 60 seconds to avoid rate limits..."
...
base_dir = Path("data/pairs_analysis_' + $timestamp + '")
results = {}
...
Write-Host "`nPair analysis complete!"
